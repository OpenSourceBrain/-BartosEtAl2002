...
Note: Not included is activation of synapses after uncoupled transient
...
def add_probabilistic_ring_projection_delay(net, presynaptic_population, pre_component, postsynaptic_population, post_component, prefix, synapse, numCells, numNeighbors, weight):
    ...
    sigma = 24  # chosen sigma to get ~57 neighbors assuming numCells=200
...
def generate_BC_cell_layer(network_id,
    ...
    if connections:
        ...
        bc_syn = ExpTwoSynapse(id="bc_syn", gbase=str(1)+"nS", erev=str(bc_syn_erev)+"mV", tau_rise="0.16 ms", tau_decay="1.8 ms")
    ...
    if generate_lems_simulation:
        ...
        if numCells_bc>0:
            ...
            of_bc = 'Volts_file_bc'
            ls.create_output_file(of_bc, "v_BC.dat")
...
if __name__ == "__main__":
    ...
    generate_BC_cell_layer("VidaTest",
                           x_size = 500,
                           y_size = 500, 
                           z_size = 500,
                           ...
                           numCells_bc = 20,
                           numNeighbors_bc = 5,
                           bc_syn_weight = 0.02,# in nS; NOTE: in original gmax_spec in mS/cm^2, f_surf = 1e-3 cm^2, so gmax = |gmax_spec| in nS   
                           bc_syn_erev = -75.,  # in mV
                           bc_gj_weight = 0.01, # in ns
                           I_mu = 1.,           # mean input drive in nA
                           I_cv = 0.1,          # std of input drive in terms of I_mu across neurons
                           connections = True,
                           gapjunx = True,
